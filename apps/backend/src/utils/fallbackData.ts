...
export function generateFallbackSeries(symbol: string, period: Period, currentPrice?: number): ISeriesData {
  console.log(`📊 Generating professional chart data for ${symbol} (${period})`);

  const basePrice = currentPrice || (50 + ((symbol.charCodeAt(0) * 7) % 800));
  const settings = PERIOD_SETTINGS[period] || PERIOD_SETTINGS['1W'];

  const isETF = symbol.includes('VOO') || symbol.includes('SPY') || symbol.includes('QQQ');
  const isVolatile = symbol.includes('TSLA') || symbol.includes('NVDA') || symbol.includes('GME');
  ...
  for (let i = 0; i < settings.points; i++) {
    ...
    const trendDirection = period === '1Y' ? 1 : (deterministicRandom() > 0.5 ? 1 : -1);
    const overallTrend = trendDirection * settings.volatility * timeProgress * 0.4;

    const wave1 = Math.sin(timeProgress * Math.PI * 1.5) * settings.volatility * 0.35;
    const wave2 = Math.sin(timeProgress * Math.PI * 4.2) * settings.volatility * 0.15;
    const smoothNoise = (deterministicRandom() - 0.5) * settings.volatility * 0.25;

    let price = basePrice * (1 + overallTrend + wave1 + wave2 + smoothNoise);

    // Keep in realistic bounds
    const boundsMultiplier = isETF ? settings.volatility * 2 : settings.volatility * 3;
    ...
    points.push({
      ...
      o: Math.round(price * (1 + (deterministicRandom() - 0.5) * 0.0005) * 100) / 100,
