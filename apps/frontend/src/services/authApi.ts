...
class AuthApiService {
  ...
  async signup(email: string, password: string, displayName?: string): Promise<IAuthResponse> {
    try {
  ...
  async generateDeviceCode(): Promise<IDeviceCodeResponse | null> {
    try {
      const response = await fetch(`${this.baseUrl}/auth/device-code/generate`, {
        method: 'POST',
      });
  ...
  async checkDeviceCodeStatus(code: string): Promise<IDeviceCodeStatusResponse | null> {
    try {
      const response = await fetch(
        `${this.baseUrl}/auth/device-code/status?code=${encodeURIComponent(code)}`
      );
