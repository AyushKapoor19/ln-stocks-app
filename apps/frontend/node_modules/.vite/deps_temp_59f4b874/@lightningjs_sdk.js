...
var getInfo = (key) => {
  const profile = { ...defaultProfile, ...Settings_default.get("platform", "profile") };
...
var normalizeColorToARGB = (color) => {
  ...
  const check2 = /^#([0-9A-F]{3}|[0-9A-F]{6})$/i;
  if (isString2(targetColor) && check2.test(targetColor)) {
...
var AppData;
var defaultOptions = {
  stage: { w: 1920, h: 1080, clearColor: 0, canvas2d: false },
  debug: false,
...
var load = async (request2) => {
  let expired = false;
  try {
    request2 = await loader(request2);
...
var TV_default = {
  ...
  addEventListener(event, cb) {
    if (typeof cb === "function") {
      ...
      Log_default.error("Please provide a function as a callback");
    }
  },
  ...
  payment(signature = {}, type = "in-app") {
    return new Promise((resolve, reject) => {
      Promise.all([
        ...
        billingRequest("/", {
          purchase: signature,
          identifier: Settings_default.get("app", "id"),
          name: Settings_default.get("app", "id"),
          household,
          country,
          operator,
          mac,
...
var PinInput = class extends Lightning_default.Component {
  static _template() {
    return {
      ...
      color: 4287927187,
      alpha: 0.5,
      shader: { type: Lightning_default.shaders.RoundedRectangle, radius: 10 },
      Nr: {
        w: (w) => w,
        y: 24,
        text: {
...
export {
  ...
  Metrics_default as Metrics,
  Pin_default as Pin,
  Profile_default as Profile,
  Purchase_default as Purchase,
